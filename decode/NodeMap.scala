...
class NodeMap(mapType: Int, inCtrl: Int, inWidth: Int, outCtrl: Int, outWidth: Int, complexity: Int) extends Module {
  ...
  out.bits := Cat(MuxCase(in.bits,
    (for (i <- 0 until mapType*(2+complexity)) yield {
       val rnd = BigInt(random.nextInt((math.pow(2, inWidth-inCtrl).toInt)))
       (in.bits > rnd.U
        && in.bits < (rnd+math.pow(2, inWidth-inCtrl).toInt).U) -> (in.bits | random.nextInt(math.pow(2, inWidth-inCtrl).toInt).U)
    }).toSeq
  ),in.bits(outCtrl-1,0))

  val cnt = complexity*2
